...
function save(isAuto) {
    ...
    setTimeout("$('.buttonLeftFixedSvDn').slideUp('slow')", interv);
...
function drawSiteItemsOption() {
    ...
    for (i = 0; i < dtaS.length; i++) {
        ...
        for (j2 = 0; j2 < crntdtaS.rssItems.length; j2++) {
            ...
            fnlRnder += '<span class="checkb"><input class="checkIsSelected floatLeft" mainId="' + crntRssItm.id + '" id="chkrssItem_' + crntRssItm.id + '" type="checkbox" onchange="markDirty();showHideControlsItems();" ' +
